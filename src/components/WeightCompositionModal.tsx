// src/components/WeightCompositionModal.tsx
import React, { useState, useEffect } from 'react';
import { View, Text, StyleSheet, TextInput, TouchableOpacity, Modal, ScrollView, Dimensions, Alert } from 'react-native';
...
interface WeightCompositionModalProps {
  visible: boolean;
  onClose: () => void;
  onSave: (data: { weight: string; bodyFat: string; weightUnit: 'kg' | 'lbs' }) => void;
  currentUser: any;
}
...
export const WeightCompositionModal: React.FC<WeightCompositionModalProps> = ({
  visible,
  onClose,
  onSave,
  currentUser
  ...
  useEffect(() => {
    if (visible && currentUser) {
      ...
      setBodyFat(currentUser.bodyFat || '');
  ...
  const loadCarouselIndex = async () => {
    try {
      const savedIndex = await AsyncStorage.getItem(`carouselIndex_${currentUser?.id}`);
      if (savedIndex) {
        setCarouselIndex(parseInt(savedIndex));
      }
    } catch (error) {
      console.error('Error loading carousel index:', error);
    }
  };

  const saveCarouselIndex = async (index: number) => {
    try {
      await AsyncStorage.setItem(`carouselIndex_${currentUser?.id}`, index.toString());
    } catch (error) {
      console.error('Error saving carousel index:', error);
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (isNaN(bodyFatValue) || bodyFatValue < minValue || bodyFatValue > maxValue) {
      setErrorMessage(`Body fat percentage must be between ${minValue}% and ${maxValue}% for ${gender}s.`);
      return;
    }
    ...
    try {
      ...
      onSave({ weight, bodyFat, weightUnit });
      setErrorMessage('');
    } catch (error) {
  ...
  const handleCarouselChange = (index: number) => {
    ...
    setBodyFat(bodyFatPercentages[index].toString());
    setErrorMessage('');
  };
  ...
  const renderBodyFatSection = () => (
    <View style={styles.inputSection}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Current Composition</Text>
